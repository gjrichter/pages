...
(function () {
	...
	ixmaps.htmlgui_onItemOver = function (evt, szId) {
		...
		console.log(themeObj.itemA)
		// insert item title and value, if theme has only one value
		if (1 || (themeObj.szFlag.match(/CHOROPLETH|VECTOR/) && themeObj.itemA[szItem] && (themeObj.itemA[szItem].nValuesA.length == 1))) {
			
			console.log(themeObj.itemA[szItem]);
			var nValue = themeObj.itemA[szItem] ? (Number(themeObj.itemA[szItem].nSize) || Number(themeObj.itemA[szItem].szValue) || themeObj.itemA[szItem].nValue || themeObj.itemA[szItem].nValuesA[0]) : "";
			var szSign = ((nValue > 0) && themeObj.szFlag.match(/SIGN/)) ? "+" : "";
			var szValue = ixmaps.__formatValue?ixmaps.__formatValue(nValue,0,"BLANK"):nValue.toFixed(themeObj.nValueDecimals||2);
			...
			if (themeObj.szFlag.match(/VECTOR/) ){
				...
				if (!themeObj.itemA[szItem].szTitle.match(/sum of/i)){
					szHtml += "<h4 style='margin-top:0em;margin-bottom:0.5em;line-height:1.2em'>" + (themeObj.itemA[szItem] ? (themeObj.itemA[szItem].szTitle || themeObj.itemA[szItem].szSelectionId2.split("::")[1] || "") : "") + "</h4>";
				}
				szHtml += "<h4 style='margin-top:0em;font-size:1.5em'>" + szSign + szValue + themeObj.szUnit + ((themeObj.itemA[szItem].nCount>1)?" <br><div style='font-size:0.8em;margin-top:0em'>"+themeObj.itemA[szItem].nCount+" progetti</div>":"")+"</h4>";
			}
			fThemeData = true;
		} else {
		...
		if (!themeObj.szFlag.match(/TOOLTIPNOCHART/)) {
			...
			if (!window.document.getElementById("chartDiv")) {
				...
				div.setAttributeNS(null, "style", "margin:-1em 1em 0em 1em;overflow:hidden");
				document.activeElement.appendChild(div);
			}
			...
			if (SVGBox && SVGBox.width && SVGBox.height) {
				...
				if (themeObj.szFlag.match(/(SYMBOL)|SEQUENCE/)) {
					width = 80 + themeObj.itemA[szItem].nValuesA.length*15;	
				} else {
		...
		if (themeObj.fShowData) {
			...
			if (data) {
				...
				for (d = 0; d < Math.min(50, data.length); d++) {
					...
					if (data.length > 1) {
						szHtml += "<tr><td style='font-size:0em;'>&nbsp;</td><tr>";
						szHtml += "<tr><td style='font-size:1em;border:solid black 1px;border-radius:1em;padding:0.1em 0.4em 0em 0.3em;text-align:left;'>"+(d+1)+"</td><tr>";
					}
					...
					for (i in dataObject) {
						...
						if (!(i == "Codice Locale Progetto" || i == "Sintesi progetto" || i == "importo" || i == "pop_2022" || i == "importo_procapite" || i == "CUP" || i == "CIG" || i == "Descrizione Submisura" || i == "Soggetto attuatore" || i == "Denominazione Aggiudicatario")){
							continue;
						}
						...
						var szValue = (isNaN(value) || !value ) ? "<span style='font-size:1em'>"+String(value)+"</span>" : "<span style='font-size:1.3em;font-weight:bold;padding-left:0.1em'>"+(ixmaps.__formatValue?ixmaps.__formatValue(value,2,"BLANK"):value.toFixed(themeObj.nValueDecimals||2)) + " " +"</span>";
						
						if ( i == "importo" ){
							szValue = " " + szValue + "€ finanziamento PNRR";
						}else
						if ( i == "importo_procapite" && !isNaN(value) ){
							szValue = " " + szValue + "€ / capite";
						}else
						if ( i == "pop_2022" ){
							szValue = " " + szValue + "abitanti";
						}else
						if ( i == "CUP" ){
							szValue = "<span style='font-size:0.8em;color:#888888'>CUP: </span><br>" + value;
						}else
						if ( i == "Codice Locale Progetto" ){
							szValue = "<span style='font-size:0.8em;color:#888888'>Codice Locale Progetto: </span><br>" + value;
						}else
						if ( i == "Sintesi progetto" ){
							szValue = "<span style='font-size:0.8em;color:#888888'>Sintesi progetto: </span><br>" + value;
						}else
						if ( (i == "CIG") && (value.length) ){
							szValue = "<p style='margin-top:0em;margin-bottom:0.5em'>CIG: " + value + "</p>";
						}else
						if ( (i == "Soggetto attuatore") && (value.length) ){
							szValue = "<span style='font-size:0.8em;color:#888888'>Attuatore: </span><br>" + szValue;
						}else
						if ( (i == "Denominazione Aggiudicatario") && (value.length) ){
							szValue = "<span style='font-size:0.8em;;color:#888888'>Aggiudicatario:</span><br>" + szValue;
						}
						...
						if (szValue.match(/http:/) || szValue.match(/https:/)) {
							...
							if (!i.match(/------/)) {
								...
								if (i == themeObj.szSizeField ||
									...
									szHtml += "<tr><td></td><td class='value' style='color:" + highLight + ";background:" + highLightBg + ";text-align:left;'>" + szValue + suffix + "</td></tr>";
								} else {
									szHtml += "<tr><td></td><td class='value' style='color:" + normal + ";text-align:left;'>" + szValue + suffix + "</td></tr>";
								}
							} else {}
						}
					}
