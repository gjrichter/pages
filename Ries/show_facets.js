...
(function () {
	...
	var __makeHistogram = function (id, szRange) { console.log(szRange);
	...
	ixmaps.data.showFacets = function (szFilter, szDiv, facetsA) {
		...
		szHtml += "<div id='list-facets' class='list-group' onmouseover='" + szHighlight + "' onmouseout='" + szClearHighlight + "'>";
			...
			if (facetsA[i].values) {
				...
				for (var ii = 0; ii < facetsA[i].values.length; ii++) {
					...
					if (facetsA[i].values[ii].length && (facetsA[i].values[ii] != " ")) {
						...
						szHtml += '<button type="button" class="btn" style="width:100%;border:none;border-bottom:solid #000000 0.1px;border-radius:0;line-height:0.9em"><div style="float:left"> <i class="icon shareIcon share_bitly icon-filter" style="float:left;margin-left:-0.5em;margin-right:1em;padding-top:0.2em;color:#888888;font-size:0.9em;display:none" title="filter by this" tabindex="-1"></i></div><div style="margin-left:-0.5em;float:left;white-space:normal;text-align:left;margin-top:0.2em;max-width:90%">' + szText + '</div><div class="pull-right" style="top:0.1em;right:-0.25em;float:right;text-align:right;font-size:18px">' + szCount + '</div></button>';
		...
		sliderA.forEach(function (x) {
			...
			var left = ($("#" + x.id).prev().offset().left + $("#" + x.id).prev().width() - $("#"+szDiv).offset().left) +20;
			//$("#" + x.id + "histogram").css("margin-left", ($("#" + x.id).prev().offset().left + $("#" + x.id).prev().width()) + "px");
			$("#" + x.id + "histogram").css("margin-left",left+"px");
			...
			var mySlider = $("#" + x.id).slider({
				...
				__setRangeFilter(x.field, $(this)[0].value, $(this).attr("data-slider-min"), $(this).attr("data-slider-max"));
			}).on("slide", function () {
				__makeHistogram($(this).attr("id"), $(this)[0].value);
			});
		});
