	...
	ixmaps.data.showFacets = function (szFilter, szDiv, facetsA) {
		...
		for (var i = 0; i < facetsA.length; i++) {
			...
			if (facetsA[i].type == "textual") {
				var placeholder = "filter by ..." + ((!facetsA[i].values) ? (" (e.g. " + (facetsA[i].example || " ") + ")") : "");
				var value = "";
				if (fActiveFacet) {
